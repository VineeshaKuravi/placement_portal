	...
	if ($result->num_rows == 0) {
		...
		if (file_exists($_FILES['resume']['tmp_name'])) {
			...
			if ($resumeFileType == "pdf") {
				...
				if ($_FILES['resume']['size'] < 100000000) { // File size is less than 5MB
		...
		$sql = "INSERT INTO users(firstname, lastname,rollno, email, password, address, city, state, contactno, qualification, stream, passingyear, dob, age, designation, resume, hash, skills) VALUES ('$firstname', '$lastname','$rollno', '$email', '$password', '$address', '$city', '$state', '$contactno', '$qualification', '$stream', '$passingyear', '$dob', '$age', '$designation', '$file', '$hash', '$skills')";
